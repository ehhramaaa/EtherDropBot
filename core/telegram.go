...
func (c *Client) sendMessage(page *rod.Page, text string, isBot bool) {
	...
	tools.Logger("success", fmt.Sprintf("| %s | Send Message %s Successfully", c.account.phone, text))
}
...
func (c *Client) gotoSetting(page *rod.Page) bool {
	...
	for attempt := 1; attempt <= 3; attempt++ {
		...
		if err != nil {
			tools.Logger("waring", fmt.Sprintf("| %s | Element Settings Not Found, Try To Find Again After 3s...", c.account.phone))
			time.Sleep(3 * time.Second)
			continue
		}
	...
	if !isClicked {
		tools.Logger("error", fmt.Sprintf("| %s | Element Settings Not Found After 3 Attempts", c.account.phone))
		return false
	}
